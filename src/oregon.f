...
C
C     THE SEED IS REDUCED TO THE VALID RANGE 1 TO MODLUS - 1, AS VALUES
C     OUTSIDE OF IT (E.G. ZERO) WOULD DEGENERATE THE GENERATOR.  THE
C     FIRST FEW VALUES OF THE SEQUENCE ARE DISCARDED BECAUSE THEY ARE
C     STILL CLOSELY CORRELATED FOR SEEDS THAT DIFFER BY LITTLE, SUCH AS
C     TIME STAMPS SECONDS APART.
C
      EXTERNAL OREGON_RAND
      REAL     OREGON_RAND
      INTEGER  ISEED, I, NWARM, MODLUS
      REAL     DUMMY
      PARAMETER (NWARM = 5, MODLUS = 2147483647)
      COMMON /SEED/ JSEED, IFRST

      JSEED = MOD(ISEED, MODLUS)
      IF (JSEED .LE. 0) JSEED = 123456789
      IFRST = 0
      DO 10 I = 1, NWARM
        DUMMY = OREGON_RAND()
   10 CONTINUE
